...
const url =
  "https://cdn.pixelbinx0.de/v2/cloudName/t.resize(h:200,w:200)/random.jpeg";
...
describe("PixelBin Image", () => {
  ...
  it("should render", async () => {
    ...
    expect(
      await screen.findByTestId("pixelbin-download-button"),
    ).toBeInTheDocument();
  ...
  it("urlObj should match expectedUrl", async () => {
    render(
      <PixelBinDownloadButton urlObj={urlObj}>Download</PixelBinDownloadButton>,
    );
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId("pixelbin-download-button").getAttribute("data-url"),
      ).toBe(expectedUrl),
    );
  });
  ...
  it("should call onDownloadStart when download starts", async () => {
    ...
    render(
      <PixelBinDownloadButton url={url} onDownloadStart={onDownloadStartMock}>
        Download
