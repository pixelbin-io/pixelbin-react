...
const _testObjectURLResponse =
  "https://cdn.pixelbin.io/v2/dummy-cloudname/original/erasebg_assets/logo/Erasebg_light_2x.png?f_auto=true";
...
describe("PixelBin Image", () => {
  ...
  it("should invoke onError when image fetch fails", async () => {
    ...
    expect(
      await screen.findByTestId("pixelbin-empty-image"),
    ).toBeInTheDocument();
  ...
  it("should invoke 'onExhausted' when retries are exhausted", async () => {
    ...
    await waitFor(() =>
      expect(onExhaustedMock).toHaveBeenCalledWith(errResponse),
    );
  });
